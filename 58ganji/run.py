...
class wubaganji_Slide_Captcha():
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) \
                      AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.81 Safari/537.36"
    }
    def get_sessionId(self,register_url):
        resp = requests.get(register_url, headers=self.headers)
        sessionId = json.loads(resp.text).get('data').get('session')
        return sessionId
    ...
    def run(self):
        ...
        sessionId = self.get_sessionId('http://vip.58ganji.com/ajax/captcha/expslider/register?act=brokerCapt')
